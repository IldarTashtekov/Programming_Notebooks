...
import part4integrations.IntegratingKafkaDStreams.{kafkaParams, kafkaTopic, spark, ssc}

import java.util

object Kafka_DStreams {
  val spark = SparkSession.builder().appName("Integrading Kafka DStreams").master("local[2]").getOrCreate()
  val ssc = new StreamingContext(spark.sparkContext, Seconds(1))

  //map between string and java object
